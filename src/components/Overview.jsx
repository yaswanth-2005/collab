...
export class Overview extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        `http://localhost:8888/patreon-clone-api/api/posts?username=${this.props.pathParam}`
      )
      .then(({ data }) => {
        this.setState({
          posts: data.message,
  ...
  handleCreatePost = ({ post_title, post_body }, e) => {
    ...
    axios
      .post("http://localhost:8888/patreon-clone-api/api/posts", {
        id_user,
        post_title,
        post_body,
      })
  ...
  handleUpdatePost = (id, e) => {
    ...
    axios
      .put(`http://localhost:8888/patreon-clone-api/api/posts/${id}`, {
        post_title,
        post_body,
      })
      .then(() => {
  ...
  handleDeletePost = (id, e) => {
    if (window.confirm("Are you sure? ")) {
      axios
        .delete(`http://localhost:8888/patreon-clone-api/api/posts/${id}`)
        .then(() => {
          this.setState(({ posts }) => ({
            posts: posts.filter((value) => id !== value.id),
          }));
        });
    }
  };
  ...
  handleFetchComments = (id, e) => {
    axios
      .get(`http://localhost:8888/patreon-clone-api/api/comments/?post=${id}`)
      .then(({ data }) => {
        this.setState({
  ...
  handleCreateComment = (id, e) => {
    ...
    axios
      .post(`http://localhost:8888/patreon-clone-api/api/comments/`, {
        id_post,
        id_user,
        comment_body,
      })
      .then(({ data }) => {
        this.setState(({ comments }) => ({
          comments: [
            ...comments,
            { id: data.message, id_post, id_user, comment_body, username },
          ],
        }));
      })
      .catch((err) => {
  ...
  handleDeleteComment = (id, e) => {
    ...
    if (window.confirm("Are you sure? ")) {
      axios
        .delete(`http://localhost:8888/patreon-clone-api/api/comments/${id}`)
        .then(() => {
          this.setState(({ comments }) => ({
            comments: comments.filter((value) => id !== value.id),
          }));
        });
    }
  };
        ...
        <React.Fragment key={value.id}>
          <Card className="mt-3">
            <Card.Body>
              ...
              <Card.Text>{value.post_body}</Card.Text>
              <Card.Link
                href={`#post${value.id}`}
              ...
              {this.props.user.username === this.props.pathParam && (
                <Card.Link href={`#action${value.id}`} data-toggle="modal">
                  action
                </Card.Link>
              )}
              ...
              >
                {this.state.comments.length > 0 ? (
                  this.state.comments.map((v) => (
                    <Card key={v.id} className="mt-2">
                      <Card.Body>
                        <Card.Text>{v.comment_body}</Card.Text>
                        <Card.Text
                ...
                <form
                  ...
                  action=""
                  method="POST"
                  onSubmit={this.handleCreateComment.bind(this, value.id)}
                >
                ...
                <div className="modal-body">
                  ...
                  <div className="tab-content pt-4">
                    <div
                      ...
                      id={`updatepost${value.id}`}
                    >
                      <form
                        action=""
                        method="POST"
                        onSubmit={this.handleUpdatePost.bind(this, value.id)}
                      >
        ...
        <Modal show={this.state.show} onHide={this.handleModal}>
          ...
          <Modal.Body>
            <form
              action=""
              method="POST"
              onSubmit={this.handleCreatePost.bind(this, this.state)}
            >
              <div className="form-group">
                <label htmlFor="Title" className="sr-only">
                  Title
